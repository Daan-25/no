            ...
            if (rows.length) {
                ...
                const fakeThread = threadRows.map(r => {
                    ...
                    return {
                        ...
                        avatar_color: r.epstein_is_sender ? '#e50914' : null,
...
function runSixDegreesSearch(query) {
    ...
    chainEl.innerHTML = '';
    const colors = ['#2980b9', '#8e44ad', '#27ae60', '#e67e22', '#e50914', '#16a085', '#d35400'];
...
async function loadArchivePage() {
    ...
    try {
        ...
        const emails = rows.map(r => ({
            ...
            ac: r.epstein_is_sender ? '#e50914' : null,
            _archive: true
        }));
...
function analyzeEmail(thread) {
    ...
    if (thread.some(m => m.avatar_color === '#e50914')) score += 10; // from Epstein
    score = Math.min(100, score);

    const scoreColor = score >= 70 ? '#e50914' : score >= 40 ? '#f39c12' : '#27ae60';
...
function renderFlightMap() {
    ...
    function drawFlights(flightsToShow, animate) {
        ...
        for (const [key, data] of Object.entries(routeCounts)) {
            ...
            L.polyline(arc, {
                color: selectedPassenger ? '#e50914' : '#3498db',
...
function generateBriefing() {
    ...
    const threatColor = dayThreat >= 5 ? '#e50914' : dayThreat >= 2 ? '#f39c12' : '#27ae60';
...
function initGlobe() {
    ...
    const dotMat = new THREE.MeshBasicMaterial({ color: 0xe50914 });
    ...
    locNames.forEach(name => {
        ...
        const ringMat = new THREE.MeshBasicMaterial({ color: 0xe50914, transparent: true, opacity: 0.4, side: THREE.DoubleSide });
